...
public class Quick {
    ...
    public static void sort(Integer[] a) {
        //StdRandom.shuffle(a); TODO: Enable this line in real usage
        showArray(a);
        sort(a, 0, a.length - 1);
        assert isSorted(a);
    }
    ...
    private static void sort(Integer[] a, int lo, int hi) {
        System.out.println();
        System.out.println();
        System.out.println("lo index = "+Integer.toString(lo));
        System.out.println("hi index = "+Integer.toString(hi));
        if (hi <= lo) return;
        int j = partition(a, lo, hi);
        System.out.println();
        System.out.println("pivot element = a["+Integer.toString(j)+"]");
        sort(a, lo, j-1);
        sort(a, j+1, hi);
    ...
    private static int partition(Integer[] a, int lo, int hi) {
        System.out.println("let's partition the array");
        int i = lo;
        int j = hi + 1;
        int pivot = a[lo];
        System.out.println("selecting pivot index = "+ lo +" which is " + a[lo]);
     ...
     */
    public static Comparable select(Integer[] a, int k) {
        if (k < 0 || k >= a.length) {
    ...
    ***************************************************************************/
    ...
    public static boolean less(int v, int w) {
        System.out.println();
        System.out.println("    less("+v+", "+w+") == "+(v < w));
        return (v < w);
    }
    ...
    private static void exch(Integer[] a, int i, int j) {
        System.out.println("\n");
        System.out.println("exchange a["+i+"] <--> a["+j+"] which are: "+a[i]+" and "+a[j]);
        Integer swap = a[i];
        a[i] = a[j];
        a[j] = swap;
    }
